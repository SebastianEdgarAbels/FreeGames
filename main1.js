...
fetch('https://www.freetogame.com/api/games', requestOptions)
  .then((response) => {
...
function createCard(games) {
  ...
  for (let i = 0; i <= games.length; i++) {
    ...
    if (myArray.length > 11) {
      let newVar = games[i].short_description.split(' ').slice(0, 10).join(' ');
      ...
      games[i].short_description.split(' ').slice(0, 10).join(' ');
