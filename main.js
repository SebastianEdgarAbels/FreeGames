...
// fetch("https://cab-cors-anywhere.herokuapp.com/https://www.freetogame.com/api/games")
//   .then((response) => {
...
createCard(data);
createOptionsVal(data);
setFilterEventListeners(data);
searchEvent(data);
...
function createCard(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (myArray.length > 11) {
      let newVar = data[i].short_description.split(" ").slice(0, 10).join(" ");
...
function showModal(data, buttonId) {
  ...
  pModalContent.innerHTML =
    data[buttonId].short_description +
    "<br>" +
    "<br>" +
    data[buttonId].developer +
    "<br>" +
    data[buttonId].publisher;
